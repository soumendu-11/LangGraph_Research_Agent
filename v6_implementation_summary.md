...
| File | Purpose |
|------|---------|
| `src/hybrid_entity_extractor.py` | 3-stage extraction logic, KNOWN_ENTITIES lookup |
| `src/hierarchical_pipeline.py` | Orchestrates ColBERT → entity extraction |
| `src/route_classifier.py` | ColBERT route classification via FAISS |
| `src/build_colbert_index.py` | Builds FAISS index from training data |
